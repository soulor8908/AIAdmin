...
describe('api/transfer 调岗域 endpoint 契约', () => {
  ...
  it('wire 适配：{code:"TRANSFER_FAILED", message:"..."} → ApiError(TRANSFER_FAILED) 含 message（AC-F3-6，T1 聚合码）', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      mockResponse(500, { code: 'TRANSFER_FAILED', message: '调岗失败：步骤 C 移除原角色失败 - role:write 权限不足' }),
    );
    globalThis.fetch = fetchMock as unknown as typeof globalThis.fetch;
  ...
  it('wire 适配：{code:"TRANSFER_SAME_ROLE"} → ApiError(TRANSFER_SAME_ROLE)（AC-F3-7，服务端兜底 superRefine）', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      mockResponse(400, { code: 'TRANSFER_SAME_ROLE', message: '新角色不能与原角色相同' }),
    );
    globalThis.fetch = fetchMock as unknown as typeof globalThis.fetch;
  ...
  it('wire 适配：{code:"USER_NOT_FOUND"} → ApiError(USER_NOT_FOUND)（AC-F3-1，T1 复用 user 域码）', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
  ...
  it('wire 适配：{code:"DEPT_NOT_FOUND"} → ApiError(DEPT_NOT_FOUND)（AC-F3-2，T1 复用 dept 域码）', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      mockResponse(404, { code: 'DEPT_NOT_FOUND', message: '部门不存在' }),
    );
    globalThis.fetch = fetchMock as unknown as typeof globalThis.fetch;
  ...
  it('wire 适配：{code:"ROLE_NOT_FOUND"} → ApiError(ROLE_NOT_FOUND)（AC-F3-3，T1 复用 role 域码）', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
  ...
  it('wire 适配：{code:"ROLE_BUILTIN_FORBIDDEN"} → ApiError(ROLE_BUILTIN_FORBIDDEN)（AC-F3-4，T1 复用 role 域码）', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      mockResponse(403, { code: 'ROLE_BUILTIN_FORBIDDEN', message: '内置角色不可移除' }),
    );
    globalThis.fetch = fetchMock as unknown as typeof globalThis.fetch;
  ...
  it('wire 适配：{code:"TRANSFER_OLD_ROLE_NOT_ASSIGNED"} → ApiError(TRANSFER_OLD_ROLE_NOT_ASSIGNED)（AC-F3-5，T1 transfer 专属码）', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      mockResponse(409, { code: 'TRANSFER_OLD_ROLE_NOT_ASSIGNED', message: '用户未持有原角色' }),
    );
    globalThis.fetch = fetchMock as unknown as typeof globalThis.fetch;
  ...
  it('wire 适配：{code:"TRANSFER_COMPENSATION_FAILED"} → ApiError(TRANSFER_COMPENSATION_FAILED)（AC-F3-6 补偿场景）', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      mockResponse(500, { code: 'TRANSFER_COMPENSATION_FAILED', message: '调岗补偿失败' }),
    );
    globalThis.fetch = fetchMock as unknown as typeof globalThis.fetch;
