...
- 校验方式：`scripts/check-rules.mjs` 正则扫描 `export\s+const\s+(\w+)\s*=\s*z\.(object|enum|array|tuple|union|intersection|record)`，对捕获的标识符检查是否以 `Schema` 结尾；不以 Schema 结尾即报 CODE-004 违规。例外清单在 `scripts/check-rules-allowlist.json`（如有）。
