...
|---|---|---|---|---|---|
| MVP | CRUD | - | - | 0 | 跑通（G1–G7 全绿），暴露 5 类系统性问题 |
| R2 | CRUD | - | - | 0 | 跑通且验证了所有复盘优化项；暴露规则声明 vs 实现漂移新问题 |
| R3 | CRUD | - | - | 0 | 跑通；META-003/004 消除漂移；暴露跨域联动代价与 test-writ |
| R4 | CRUD | - | - | 0 | 跑通；三个 P1 反推优化点全部验证生效；跨域联动实现零测试改动/零救火/可预测 |
| R5 | CRUD | - | - | ? | 跑通（经 1 轮 blocker 修复）；三个架构方向全部落地；首次暴露测试通过 |
| R6 | CRUD | - | - | 0 | 跑通；第五轮反推的三项优化点全部验证生效；验收对齐门禁首次闭合，Reviewer |
| R7 | CRUD | - | - | 0 | 跑通；第六轮反推的更复杂业务适应性建议验证生效；新架构模式（多步事务+补偿+事务 |
| R8 | CRUD | 23 | - | 0 | 跑通；第七轮反推的更复杂业务适应性建议验证生效（权限继承方向）；递归继承 + 环 |
| R9 | CRUD | 18 | - | 0 | 跑通；第八轮反推的权限继承方向已落地（R8）；本期转向HTTP 协议扩展 + 并 |
| R10 | CRUD | 17 | - | 0 | 跑通；R9 落地 If-Match 写条件，本期落地 If-None-Match |
| R11 | CRUD | 23 | - | 0 | 跑通；R10 落地 ETag 读条件（单层变更），本期落地鉴权域（新业务域 +  |
| R12 | CRUD | - | - | 0 | 跑通；ARCH-003 闭合；wire 适配 advisory 范例；多约束组合 |
| R13 | 元改进 | - | - | 0 | S-1~S-4 固化 + S-5 记录未来方向；元改进轮不走标准 PRD→Spe |
| R14 | CRUD | - | - | 0 | 跑通；多域适应性验证；R13 固化提示词首次大规模验证；①类显式影响处理范例 |
| R15 | CRUD | - | - | 0 | 跑通；七域全覆盖；client.ts 扩展范例；通知状态机+versioned  |
| R16 | CRUD | - | - | 0 | 跑通；后端能力前端化闭合；errorMapping 全码映射收尾；事务性+继承链 |
| R17 | 元改进 | - | - | 0 | 跑通；6 项遗留 S 级教训全部固化；S-6 探针验证（lint:rules e |
| R18 | 业务 | 24 | 1256 | 0 | 跑通；D10/D19 两项历史 advisory 偏离消除 + D9 重分类 [ |
| R19 | 元改进 | - | - | 0 | 跑通；R18 S-21/S-22 两项教训全部固化至提示词层（impl-writ |
| R20 | 基础设施 | 19 | 1256 | 0 | 跑通；19 AC 全对齐（AC-E1~E14 共 14 条 E2E + AC-S |
| R21 | 元改进 | - | - | 0 | 跑通；R20 S-23 一项教训全部固化至提示词层（Tech Lead + BA |
| R22 | 业务 | 15 | 1263 | 0 | 跑通；15 AC 全对齐（AC-P1~P10 性能 10 条 + AC-S20- |
| R23 | 业务 | 11 | 1272 | 0 | 跑通；11 AC 全对齐（AC-A11y-1~11）；三件套全绿（typeche |
| R24 | 基础设施 | 9 | 1272 | 0 | 跑通；9 AC 全对齐（AC-XB-1~9，8 pass + 1 conditi |
